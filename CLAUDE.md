...
- Route handlers that catch errors must re-throw on `isFrameworkSignal(error)` (see `src/app/api/admin/calendar/route.ts`) — otherwise `redirect()`, `notFound()`, `unauthorized()`, and `forbidden()` thrown deeper in the call chain get silently 500'd.
